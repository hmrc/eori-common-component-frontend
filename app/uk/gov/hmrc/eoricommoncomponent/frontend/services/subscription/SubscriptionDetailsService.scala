...
class SubscriptionDetailsService @Inject() (
  ...
  def cacheNameIdCustomsIdAndUtrMatch(name: String, id: String, utrMatch: Option[UtrMatchModel])(implicit
    hc: HeaderCarrier
  ): Future[Unit] =
  ...
  def cacheEoriNumber(eoriNumber: String)(implicit hc: HeaderCarrier): Future[Unit] =
    saveSubscriptionDetails(sd => sd.copy(eoriNumber = Some(FormUtils.fixUserInput(eoriNumber))))
