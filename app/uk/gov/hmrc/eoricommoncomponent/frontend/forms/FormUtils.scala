...
object FormUtils {
  ...
  def fixUserInput(value: String): String                      = value.replaceAll(" ", "").toUpperCase
  def fixUserInput(maybeValue: Option[String]): Option[String] = maybeValue.map(value => fixUserInput(value))
