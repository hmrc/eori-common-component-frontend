...
class SubscriptionOutcomeFailPartnershipSpec extends ViewSpec {

  implicit val request = withFakeCSRF(fakeAtarSubscribeRequest)

  private val view = instanceOf[subscription_outcome_fail_partnership]
  ...
  "'Subscription Fail' Page" should {
    ...
    "have the correct h2 for company" in {
      doc().body.getElementById("orgType").text must startWith(
        messages("cds.subscription.outcomes.rejected.heading2.llp")
      )
    }
    "have correct partnership paragraph" in {
      doc().body.getElementById("partnership-para")
    }

    "have a companies house link" in {
      val link = doc().body.getElementById("partnership-link")
      link.text mustBe messages("cds.navigation.hmrc.online-services")
      link.attr("href") mustBe "https://www.gov.uk/log-in-register-hmrc-online-services"
    }

    "have the correct h2 for corporation" in {
      doc().body.getElementById("corporation-heading").text must startWith(messages("cds.utr.label"))
    }
    "have the correct para 3 " in {
      doc().body.getElementById("rejected-para3").text must startWith(
        messages("cds.subscription.outcomes.rejected.para3.llp")
      )
    }
