...
class SubscriptionOutcomeFailOrganisationSpec extends ViewSpec {

  implicit val request = withFakeCSRF(fakeAtarSubscribeRequest)

  private val view = instanceOf[subscription_outcome_fail_organisation]
  ...
  "'Subscription Fail' Page" should {
    ...
    "have the correct h2 for company" in {
      doc().body.getElementById("orgType").text must startWith(
        messages("cds.subscription.outcomes.rejected.heading2.organisation")
      )
    }

    "have the correct h3 for charity" in {
      doc().body.getElementById("charity-heading").text must startWith(
        messages("cds.subscription.outcomes.rejected.heading-h3.organisation")
      )
    }
    ...
    "have the correct para 3 " in {
      doc().body.getElementById("rejected-para3").text must startWith(
        messages("cds.subscription.outcomes.rejected.para3.organisation")
      )
    }
