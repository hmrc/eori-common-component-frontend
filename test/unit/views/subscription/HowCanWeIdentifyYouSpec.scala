...
class HowCanWeIdentifyYouSpec extends ViewSpec {
  ...
  lazy val doc: Document = Jsoup.parse(contentAsString(view(form, isInReviewMode, atarService)))

  lazy val docWithRadioButtonsError: Document =
    Jsoup.parse(contentAsString(view(formWithNothingSelected, isInReviewMode, atarService)))
