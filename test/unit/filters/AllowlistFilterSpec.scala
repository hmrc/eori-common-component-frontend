...
class AllowlistFilterSpec extends UnitSpec with MockitoSugar with BeforeAndAfterEach with GuiceOneAppPerSuite {
  ...
  "AllowlistFilter on restricted route" should {
    "Do nothing" in {
      ...
      implicit val request = FakeRequest("GET", "/customs-enrolment-services/register-for-cds").withHeaders(HeaderNames.REFERER -> "123")
  ...
  "AllowlistFilter on permitted route" should {
    val requestOnPermittedRoute = FakeRequest("GET", "/customs-enrolment-services/subscribe-for-cds")
    ...
    "Append Session Param on permitted child route with matched referrer" in {
      ...
      implicit val request =
        FakeRequest("GET", "/customs-enrolment-services/subscribe-for-cds/some-path").withHeaders(HeaderNames.REFERER -> "test")
