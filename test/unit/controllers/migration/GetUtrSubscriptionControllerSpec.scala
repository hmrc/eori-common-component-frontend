  ...
  "HaveUtrSubscriptionController createForm" should {
    "return OK and display correct page when orgType is Company" in {
      ...
      createForm() { result =>
        ...
        page.title() should include("Your Corporation Tax Unique Taxpayer Reference (UTR)")
      }
    }
    ...
    "return OK and display correct page when orgType is Sole Trader" in {
      ...
      createForm() { result =>
        status(result) shouldBe OK
        val page = CdsPage(contentAsString(result))
        page.title() should include("Your Self Assessment Unique Taxpayer Reference (UTR)")
      }
    }
