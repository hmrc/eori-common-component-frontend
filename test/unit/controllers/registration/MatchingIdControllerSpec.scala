...
class MatchingIdControllerSpec extends ControllerSpec with BeforeAndAfterEach {
  ...
  "MatchingIdController for Subscribe Journey" should {
    ...
    "redirect to GG login when request is not authenticated with redirect to based in UK" in {
      ...
      status(result) shouldBe SEE_OTHER
      result.header.headers("Location") shouldBe "/gg/sign-in?continue=http%3A%2F%2Flocalhost%3A6750%2Fcustoms%2Fsubscribe-for-cds%2Fare-you-based-in-uk&origin=eori-common-component-frontend"
    }
    ...
    "redirect to GG login when request is not authenticated with redirect to Subscribe when the user selects yes on based in uk" in {
      ...
      status(result) shouldBe SEE_OTHER
      result.header.headers("Location") shouldBe "/gg/sign-in?continue=http%3A%2F%2Flocalhost%3A6750%2Fcustoms%2Fsubscribe-for-cds%2Fsubscribe&origin=eori-common-component-frontend"
    }
