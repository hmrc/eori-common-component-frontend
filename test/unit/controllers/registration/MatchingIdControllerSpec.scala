  ...
  "MatchingIdController for Subscribe Journey" should {
      ...
      val result = controller
        ...
        .apply(
          SessionBuilder.buildRequestWithSessionAndPathNoUserAndBasedInUkNotSelected(
            method = "GET",
            path = "/eori-common-component/subscribe-for-cds/subscribe"
          )
        )
      status(result) shouldBe SEE_OTHER
      result.header.headers("Location") shouldBe "/gg/sign-in?continue=http%3A%2F%2Flocalhost%3A6750%2Feori-common-component%2Fsubscribe-for-cds%2Fare-you-based-in-uk&origin=eori-common-component-frontend"
    }
    ...
    "redirect to GG login when request is not authenticated with redirect to Subscribe when the user selects yes on based in uk" in {
      ...
      val result = controller
        ...
        .apply(
          SessionBuilder
            .buildRequestWithSessionAndPathNoUser(method = "GET", path = "/eori-common-component/subscribe-for-cds/subscribe")
        )
      status(result) shouldBe SEE_OTHER
      result.header.headers("Location") shouldBe "/gg/sign-in?continue=http%3A%2F%2Flocalhost%3A6750%2Feori-common-component%2Fsubscribe-for-cds%2Fsubscribe&origin=eori-common-component-frontend"
    }
